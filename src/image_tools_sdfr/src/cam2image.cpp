...
private:
  ...
  void initialize()
  {
    ...
    pub_ = create_publisher<image_tools_sdfr::ROSCvMatContainer>("image", qos);
  ...
  void timerCallback()
  {
    ...
    image_tools_sdfr::ROSCvMatContainer container(frame, header);
    ...
    if (DEBUG)
    {
      ...
      RCLCPP_INFO_THROTTLE(get_logger(), clk, 1000, "Publishing image #%zd", publish_number_);
    }
  ...
  void parse_parameters()
  {
    ...
    rcl_interfaces::msg::ParameterDescriptor remote_mode_desc;
    remote_mode_desc.description = "Gets images from a remote camera instead of a camera on the device";
    remote_mode_ = this->declare_parameter("remote_mode", false, remote_mode_desc);
    frame_id_ = this->declare_parameter("frame_id", "camera_frame");

    rcl_interfaces::msg::ParameterDescriptor socket_ip_desc;
    socket_ip_desc.description = "The IP that will be used when opening a socket connection to a remote camera";
    socket_ip_ = this->declare_parameter("socket_ip", "127.0.0.1", socket_ip_desc);

    rcl_interfaces::msg::ParameterDescriptor socket_port_desc;
    socket_port_desc.description = "The port that will be used when opening a socket connection to a remote camera";
    socket_port_ = this->declare_parameter("socket_port", 80, socket_port_desc);
  }
  ...
  rclcpp::Publisher<image_tools_sdfr::ROSCvMatContainer>::SharedPtr pub_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(image_tools_sdfr::Cam2Image)
